...
static void mavlink_test_thermo_gyro_switch(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	mavlink_thermo_gyro_switch_t packet_in = {
		963497464,
	}17,
	};
	mavlink_thermo_gyro_switch_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        	packet1.time_boot_ms = packet_in.time_boot_ms;
        	packet1.control_switch = packet_in.control_switch;
        
        

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_thermo_gyro_switch_encode(system_id, component_id, &msg, &packet1);
	mavlink_msg_thermo_gyro_switch_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_thermo_gyro_switch_pack(system_id, component_id, &msg , packet1.time_boot_ms , packet1.control_switch );
	mavlink_msg_thermo_gyro_switch_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_thermo_gyro_switch_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.time_boot_ms , packet1.control_switch );
	mavlink_msg_thermo_gyro_switch_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_thermo_gyro_switch_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_thermo_gyro_switch_send(MAVLINK_COMM_1 , packet1.time_boot_ms , packet1.control_switch );
	mavlink_msg_thermo_gyro_switch_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_ardupilotmega(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_thermo_gyro_switch(system_id, component_id, last_msg);
}
