...
static NOINLINE void send_thermo_gyro_switch(mavlink_channel_t chan)
{
    mavlink_msg_thermo_gyro_switch_send(
        chan,
        millis(),
        controller_info.control_type);
}
...
static bool mavlink_try_send_message(mavlink_channel_t chan, enum ap_message id)
{
    ...
    case MSG_THERMO_GYRO_SWITCH:
        CHECK_PAYLOAD_SIZE(THERMO_GYRO_SWITCH);
        send_thermo_gyro_switch(chan);
        break;
...
GCS_MAVLINK::data_stream_send(void)
{
    ...
    if (stream_trigger(STREAM_EXTRA3)) {
        ...
        send_message(MSG_THERMO_GYRO_SWITCH);
    }
}
...
void GCS_MAVLINK::handleMessage(mavlink_message_t* msg)
{
    ...
    case MAVLINK_MSG_ID_COMMAND_LONG:
    {
        ...
        case MAV_CMD_DO_THERMO_GYRO_SWITCH:
            if ((packet.param1 == 1)&&(packet.param2 == 0)) {
                 controller_info.control_type = 1; //This means the gyro is in control
                 result = MAV_RESULT_ACCEPTED;
            } else if ((packet.param1 == 0)&&(packet.param2 == 1)) {
                 controller_info.control_type = 2; //This means the thermo is in control
                 result = MAV_RESULT_ACCEPTED;
            }
            break;
