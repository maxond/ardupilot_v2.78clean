...
static void startup_INS_ground(bool do_accel_init)
{
    ...
    if (thermo_sensors.enabled()) {
        thermo_sensors.calibrate();
        gcs_send_text_P(SEVERITY_LOW,PSTR("THERMO SENSORS CALIBRATED!!!"));
    } else {
        gcs_send_text_P(SEVERITY_LOW,PSTR("NO Thermo Sensor"));
    } 
}
