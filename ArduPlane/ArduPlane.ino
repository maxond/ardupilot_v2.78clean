...
AP_HAL::AnalogSource * thermo_pin_x;
AP_HAL::AnalogSource * thermo_pin_y;
AP_HAL::AnalogSource * thermo_pin_z; 
...
////////////////////////////////////////////////////////////////////////////////
AP_Thermo_Sensors thermo_sensors;
static struct {
  int32_t roll_sensor;
...
static struct {
    ...
    control_type : 1  // Gyro in control
};

Vector3f raw_thermo_reading = Vector3f(0,0,0);
Vector3f raw_thermo_reading_prev = Vector3f(0,0,0);
int thermo_max        = 300;    // used to scale Thermopile output to 511
int thermo_max_save   = 300;    // used to scale Thermopile output to 511
...
void setup() {
    ...
    thermo_pin_x = hal.analogin->channel(THERMO_PIN_X);
    thermo_pin_y = hal.analogin->channel(THERMO_PIN_Y);
    thermo_pin_z = hal.analogin->channel(THERMO_PIN_Z);

    thermo_sensors.init(thermo_pin_x,thermo_pin_y,thermo_pin_z);    
